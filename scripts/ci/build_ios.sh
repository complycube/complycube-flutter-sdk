...
# Usage:
#   ./scripts/ci/build_ios.sh --row-json scripts/ci/matrix_row.json --results-dir artifacts/results
...
if [[ "$outcome" == "pass" ]]; then
  set +e
  (cd ios && pod install) 2>&1 | tee "$build_log"
  pod_status=$?
  set -e
...
if [[ "$outcome" == "pass" ]]; then
  set +e
  flutter build ios --no-codesign 2>&1 | tee -a "$build_log"
  build_status=$?
  set -e
...
detected="$(./scripts/ci/collect_versions.sh --platform ios --row-id "$row_id")"
