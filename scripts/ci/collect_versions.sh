...
python3 - "$ROW_ID" "$runner_os" "$flutter_version" "$dart_version" "$java_version" "$agp_version" "$kotlin_version" "$gradle_version" "$android_triplet" "$xcode_version" "$swift_version" "$cocoapods_version" "$ruby_version" <<'PY'
import json, sys, time
(row_id, runner_os, flutter_version, dart_version, java_version,
 agp_version, kotlin_version, gradle_version, android_triplet,
 xcode_version, swift_version, cocoapods_version, ruby_version) = sys.argv[1:14]
print(json.dumps({
  "row_id": row_id,
  "runner_os": runner_os,
  ...
  "android_sdk_triplet": android_triplet,
